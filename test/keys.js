...
test('triangle key equality, one string docs', t => {
  uusym.reset()
  const a = uusym('A basic test object, not really expected to be unique')
  const b = uusym('A basic test object, not really expected to be unique')
  const c = uusym('Another basic test object, not really expected to be unique, but still different from some')
  const ka = a.key
  const kb = b.key
  const kc = c.key
  t.equal(a.key, ka)
